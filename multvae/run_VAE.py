...
#DATASET = './processed_RetailRocket'
...
def run_with_tracking(model, opts, train_data, batch_size, n_epochs, beta, gamma, dropout_rate):
    ...
    '''
    run_with_progress_tracking(model, opts, train_data, batch_size, n_epochs, beta, gamma, dropout_rate)
...
def run_with_progress_tracking(model, opts, train_data, batch_size, n_epochs, beta, gamma, dropout_rate):
    '''
    Función que entrena el modelo en un conjunto de datos durante un número de épocas,
    mostrando el progreso del entrenamiento por época y batch.
    '''
    model.train()
    ...
    for epoch in range(n_epochs):
        ...
        total_batches = int(np.ceil(train_data.shape[0] / batch_size))
        
        for batch_idx, batch in enumerate(
            generate(batch_size=batch_size, device=device, data_in=train_data, shuffle=True)
        ):
            ...
            estimated_remaining_time = remaining_batches * elapsed_batch_time

            print(
                f"Batch {batch_idx + 1}/{total_batches} | "
                f"Loss: {loss.item():.4f} | "
                f"Batch time: {elapsed_batch_time:.2f}s | "
    ...
    print(f"\nTotal training time: {total_training_time / 60:.2f} min\n{'='*40}\n")
